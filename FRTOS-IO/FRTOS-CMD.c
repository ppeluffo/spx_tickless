...
uint8_t i;
	...
	for ( i = 0; i < BDCMD_numCommands; i++) {
		snprintf_P( d_printfBuff,sizeof(d_printfBuff),PSTR("REG[%d]: %s\r\n\0"),i, BDCMD_commandList[i]);
...
uint8_t cmdIndex;
	...
	// Muestro el cmdLine
	//snprintf_P( d_printfBuff,sizeof(d_printfBuff),PSTR("CL[%d]: %s\r\n\0"),i, cmdLine_buffer);
	//CMD_write(d_printfBuff, sizeof(d_printfBuff) );
	//
	...
	for( cmdIndex = 0; cmdIndex < BDCMD_numCommands; cmdIndex++) {
		...
		//snprintf_P( d_printfBuff,sizeof(d_printfBuff),PSTR("LN[%d]: %s\r\n\0"),cmdIndex, BDCMD_commandList[cmdIndex]);
		//CMD_write(d_printfBuff, sizeof(d_printfBuff) );
