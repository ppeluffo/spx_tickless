...
#ifdef DEBUG_I2C
		snprintf_P( debug_printfBuff,CHAR128,PSTR("FRTOS_I2C_RD: devAddr:0x%02x,addrLen:0x%02x,byteAddr:0x%02x,xbytes: 0x%02x\r\n\0"),pI2C->devAddress, pI2C->byteAddressLength, pI2C->byteAddress, xBytes);
		CMD_write( debug_printfBuff, sizeof(debug_printfBuff) );
#endif
...
uint16_t *p;
		...
		#ifdef DEBUG_SPI
				snprintf_P( debug_printfBuff,CHAR128,PSTR("FRTOS_SPI_IOCTL: 0x%02x,0x%02x\r\n\0"),(uint8_t)ulRequest, (uint8_t)(*p));
				CMD_write( debug_printfBuff, sizeof(debug_printfBuff) );
		#endif
